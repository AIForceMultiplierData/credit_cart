...
function StatCard({
  ...
  return (
    <${D} className="rounded-2xl border border-slate-800/50 bg-slate-900/60 p-4 text-center backdrop-blur-md">
...
export function HomeView({ onNavigate }: HomeViewProps) {
  ...
  return (
    <${D} className="px-4 pb-32 pt-2">
      <${D} className="relative mb-6 overflow-hidden rounded-3xl">
        <${D} className="absolute inset-0 bg-gradient-to-br from-emerald-500/20 via-blue-500/10 to-purple-500/20" />
        <${D} className="relative p-6">
          <${D} className="mb-3 flex items-center gap-2">
            <${D} className="flex h-10 w-10 items-center justify-center rounded-full bg-emerald-400/20">
      ...
      {isNewUser && (
        <${D} className="mb-6 rounded-2xl border border-emerald-500/20 bg-emerald-500/10 p-4 text-center">
      ...
      <${D} className="mb-6 grid grid-cols-2 gap-3">
        <StatCard
          icon={TrendingUp}
      ...
      <${D} className="space-y-3">
        ...
        >
          <${D} className="flex items-center gap-3">
            <${D} className="flex h-12 w-12 items-center justify-center rounded-xl bg-emerald-500/20">
              <Zap className="h-6 w-6 text-emerald-400" />
            </${D}>
            <${D} className="text-left">
        ...
        >
          <${D} className="flex items-center gap-3">
            <${D} className="flex h-12 w-12 items-center justify-center rounded-xl bg-blue-500/20">
        ...
        >
          <${D} className="flex items-center gap-3">
            <${D} className="flex h-12 w-12 items-center justify-center rounded-xl bg-purple-500/20">
...
const outPath = path.join(__dirname, "..", "components", "home-view.tsx");
const finalContent = content.replace(/motionlessPLACEHOLDER/g, "motionlessmotionlessmotionlessmotionlessmotionlessdiv");
fs.writeFileSync(outPath, finalContent.replace(/motionlessmotionlessmotionlessmotionlessmotionlessdiv/g, "div"), "utf8");

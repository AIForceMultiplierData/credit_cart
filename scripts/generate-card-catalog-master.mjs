...
const cardsRaw = CARD_ROWS.map(([slug, name, image, color, styles, order]) => {
...
for (const card of cardsRaw) {
  if (seenSlugs.has(card.card_slug)) continue
  seenSlugs.add(card.card_slug)
  cards.push(card)
}
...
--
-- Clears duplicate slug rows, then reloads ${cards.length} cards in one shot.

delete from public.card_catalog;
...
begin
  ...
  if card_id_type = 'uuid' then
    ...
    select distinct on (v.card_slug)
      v.bank_name, v.card_name, v.style_classes, v.card_slug, v.bank_id, v.card_image_url, true
    from (values
${valueLines.join(",\n")}
    ) as v(bank_name, card_name, style_classes, card_slug, bank_id, card_image_url)
    order by v.card_slug;
  else
    insert into public.card_catalog (card_id, bank_name, card_name, style_classes, card_slug, bank_id, card_image_url, is_active)
    select distinct on (v.card_slug)
      v.card_slug, v.bank_name, v.card_name, v.style_classes, v.card_slug, v.bank_id, v.card_image_url, true
    from (values
${valueLines.join(",\n")}
    ) as v(bank_name, card_name, style_classes, card_slug, bank_id, card_image_url)
    order by v.card_slug
...
--   3. supabase/card_catalog_live_seed.sql — DELETE + reload ${cards.length} cards
--

    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-hidden border-slate-800 bg-slate-950/95 p-0 text-slate-50 backdrop-blur-xl sm:max-w-md">
        ...
        <TAG_DIV className="max-h-[55vh] overflow-y-auto px-5 py-4">
          {catalogLoading ? (
            <TAG_DIV className="flex min-h-[220px] flex-col items-center justify-center gap-3 rounded-xl border border-slate-800/60 bg-slate-900/40 backdrop-blur-md">
              <Loader2 className="h-8 w-8 animate-spin text-emerald-400" />
              <p className="text-sm text-slate-400">Loading card catalog…</p>
            </TAG_DIV>
          ) : catalogError ? (
            <TAG_DIV className="rounded-xl border border-red-500/30 bg-red-950/20 p-6 text-center backdrop-blur-md">
              <p className="font-medium text-red-300">Could not load catalog</p>
              <p className="mt-1 text-sm text-slate-400">{catalogError}</p>
            </TAG_DIV>
          ) : catalogData.length === 0 ? (
            <TAG_DIV className="rounded-xl border border-dashed border-slate-700 bg-slate-900/40 p-8 text-center backdrop-blur-md">
